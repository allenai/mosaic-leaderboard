...
class AccuracyBreakdown:
    ...
    def __str__(self):
        ...
        for cat in self.categories():
            string += accuracy_string(cat, self.category_correct(cat),
                                      self.category_total(cat)) + '\n'
        return string
    ...
    def category_accuracy(self, cat):
        return self._category_correct[cat] / float(self._category_total[cat])
...
def main(args):
    ...
    if len(preds) != len(answers):
        raise Exception(
            "The prediction file does not contain the same number of lines as the number of test instances.")
...
def get_parser():
    ...
    # Required Parameters
    parser.add_argument('--answers_file', type=str, help='Location of test answers', default=None,
                        required=True)
    ...
    parser.add_argument("--questions_file", type=str,
                        help="Location of test questions. Pass this parameter to get accuracy broken down by question categories.")
    return parser
